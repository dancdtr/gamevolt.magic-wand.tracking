...
class WandSpellCueController:
    ...
    def _on_spell_cast(self, cast: SpellCast) -> None:
        asyncio.create_task(self._handle_cast(cast))

    async def _handle_cast(self, cast: SpellCast) -> None:
        try:
            result = await self._spell_cast_reporter.report_spell_cast(cast)
        except Exception:
            self._logger.exception(f"Spell cast report failed for wand ({cast.wand_id})")
            return

        self._wand_device_controller.play_spell_cast_cue(cast.wand_id, result.quality)
