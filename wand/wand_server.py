...
class WandServer(WandServerProtocol):
    """
    Orchestrates:
      SerialReceiver -> Parser -> Assembler -> Registry -> WandClient
    """
    ...
    def _on_line(self, line: str) -> None:
        ...
        self._logger.debug(
            f"DATA: seq={pkt.seq} tag={pkt.tag_hex} t_base={pkt.t_base_ms} nsamp={pkt.nsamp} "
            f"hdr_age_s={pkt.header_age_s:.3f} data_len={len(pkt.data_str)} data_preview='{preview}'"
        )
        ...
        try:
            client.on_wand_rotation_data(pkt.t_base_ms, pkt.data_str)
        except Exception:
            self._logger.exception(
                f"Exception while routing wand data: tag={pkt.tag_hex} seq={pkt.seq} "
                f"t_base={pkt.t_base_ms} data_len={len(pkt.data_str)}"
            )

    def _on_wand_rotation_raw_updated(self, msg: WandRotationRaw) -> None:
        self._logger.debug(f"wand_rotation_raw_updated: id={msg.id} ts={msg.ms} yaw={msg.yaw:.3f} pitch={msg.pitch:.3f}")
