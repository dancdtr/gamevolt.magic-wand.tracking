...
class WandDeviceController:
    ...
    def play_spell_cast_cue(self, wand_id: str, has_sufficient_level: bool) -> None:
        ...
        # Space the lamp cue away from the haptic. Back-to-back CMD1 in the same
        # TX burst lets the hwave state mask the subsequent blink. Same trick as
        # `activate_wand` uses between TX-enable and the chosen haptic.
        self._delay(
            self._settings.command_broadcast_interval,
            lambda: self.play_spell_cast_lamp_cue(wand_id),
        )

    def play_spell_cast_lamp_cue(self, wand_id: str) -> None:
        self._logger.verbose(f"Pulsing wand ({wand_id}) lamp on spell cast...")
        self._command_sink.send_to_wand(wand_id, WandLedMessage(wand_id, enabled=True, sequence_id=0))
        self._delay(
            self._settings.spell_cast_lamp_duration_s,
            lambda: self._command_sink.send_to_wand(
                wand_id, WandLedMessage(wand_id, enabled=False, sequence_id=0)
            ),
        )
