...
instruction_settings = ImageProviderSettings(bundled_path("assets", "spells"), 500, (255, 255, 255), (0, 0, 0))
success_settings = ImageProviderSettings(bundled_path("assets", "spells"), 500, (0, 255, 0), (0, 0, 0))
spell_image_library_settings = SpellImageLibrarySettings(instruction_settings, success_settings)

image_library_settings = ImageLibrarySettings(bundled_path("assets", "gestures"), 300)

udp_tx = UdpTx(logger, settings.spells_udp.udp_transmitter)
udp_rx = UdpRx(logger, settings.spells_udp.udp_receiver)
...
application_dir = os.path.dirname(os.path.abspath(__file__))
config_path = bundled_path("appsettings.yml")
config_env_path = install_path(application_dir, "appsettings.env.yml")
settings = AppSettings.load(config_file_path=config_path, config_env_file_path=config_env_path)
